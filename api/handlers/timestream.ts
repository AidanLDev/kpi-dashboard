...
function parseRows(columns: ColumnInfo[], rows: Row[]): UserActivityRow[] {
  return rows.map((row) =>
    Object.fromEntries(
      (row.Data ?? []).map((cell: { NullValue?: boolean; ScalarValue?: string }, i: number) => [
...
const EXCLUDED_IDS_SQL = EXCLUDED_USER_IDS.map((id) => `'${id}'`).join(",\n          ");

async function getTimestreamData(
  startDate: string,
  endDate: string,
...
async function getTimestreamDataByDate(
  startDate: string,
  endDate: string,
  ...
  const result = await client.send(
    new QueryCommand({
      QueryString: `
        SELECT DATE(time) AS day, COUNT(DISTINCT userId) AS userCount
        FROM "LineVuPortalUserActivityDatabase-prod"."UserActivity-prod"
        WHERE time between '${startDate} 00:00:00' and '${endDate} 23:59:59'
        AND userId NOT IN (
          ${EXCLUDED_IDS_SQL}
  ...
  for (const row of result.Rows ?? []) {
    ...
    const day = cells[0]?.ScalarValue ?? "";
    const count = Number(cells[1]?.ScalarValue ?? "0");
    if (day) record[day] = count;
  }
  return record;
}
...
export const handler = async (
  ...
  try {
    ...
    return {
      statusCode: 200,
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ rows, byDate }),
    };
  } catch (err) {
    console.error("[timestream handler error]", err);
    return {
      statusCode: 500,
      headers: { "Content-Type": "application/json" },
